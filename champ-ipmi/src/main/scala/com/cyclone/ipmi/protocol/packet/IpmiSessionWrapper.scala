...
object IpmiSessionWrapper {
  def encode(wrapper: IpmiSessionWrapperRequest): ByteString =
    wrapper match {
      case wrapper: Ipmi20SessionWrapper.Request => Ipmi20SessionWrapper.Request.coder.encode(wrapper)
      case wrapper: Ipmi15SessionWrapper.Request => Ipmi15SessionWrapper.Request.coder.encode(wrapper)
    }

  def decode(data: ByteString, sessionContext: SessionContext): IpmiErrorOr[IpmiSessionWrapperResponse] = {
  ...
  def wrapperFor[P <: IpmiRequestPayload : Coder](
    ...
    sessionContext: SessionContext): IpmiSessionWrapperRequest = {

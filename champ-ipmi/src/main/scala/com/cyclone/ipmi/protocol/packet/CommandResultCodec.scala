...
object CommandResultCodec {
  ...
  def commandResultCodecFor[Cmd <: IpmiCommand: Coder, Res <: IpmiCommandResult: Decoder](
    implicit statusCodeTranslator: StatusCodeTranslator[Res]
  ): CommandResultCodec[Cmd, Res] =
    CommandResultCodec(implicitly[Coder[Cmd]], implicitly[Decoder[Res]], statusCodeTranslator)
}

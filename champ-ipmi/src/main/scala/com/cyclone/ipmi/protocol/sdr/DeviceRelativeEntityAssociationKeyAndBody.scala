...
/**
  ...
  */
case class DeviceRelativeEntityAssociationKeyAndBody(data: ByteString) extends SdrKeyAndBody {
  val sensorIds: Seq[SensorId] = Nil
  val sensorNumbers: Seq[SensorNumber] = Nil

  val recordType: SensorDataRecordType.EntityAssociation.type =
    SensorDataRecordType.EntityAssociation
  val optSensorType: Option[SensorType] = None
}

object DeviceRelativeEntityAssociationKeyAndBody {
  implicit val decoder: Decoder[DeviceRelativeEntityAssociationKeyAndBody] =
    new Decoder[DeviceRelativeEntityAssociationKeyAndBody] {

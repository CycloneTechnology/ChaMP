...
object GetSystemInfoParameterSelectors {
  ...
  case object Guid extends ParameterSelector {
    ...
    implicit val parameterCodec: ParameterCodec[Guid.type, Data, Nothing] =
      ParameterCodec.parameterCodecFor[Guid.type, Data]
  }

  case object AssetTag extends ParameterSelector {
    val code = 0xc4

    implicit val parameterCodec: ParameterCodec[AssetTag.type, ShortString, Nothing] =
      ParameterCodec.parameterCodecFor[AssetTag.type, ShortString]
  }

  case object ServiceTag extends ParameterSelector {
    val code = 0xc5

    implicit val parameterCodec: ParameterCodec[ServiceTag.type, ShortString, Nothing] =
      ParameterCodec.parameterCodecFor[ServiceTag.type, ShortString]
  }

  case object ChassisServiceTag extends ParameterSelector {
    val code = 0xc6

    implicit val parameterCodec: ParameterCodec[ChassisServiceTag.type, ShortString, Nothing] =
      ParameterCodec.parameterCodecFor[ChassisServiceTag.type, ShortString]
  }

  case object ChassisRelatedServiceTag extends ParameterSelector {
    val code = 0xc7

    implicit val parameterCodec: ParameterCodec[ChassisRelatedServiceTag.type, ShortString, Nothing] =
      ParameterCodec.parameterCodecFor[ChassisRelatedServiceTag.type, ShortString]
  }

  case object BoardRevision extends ParameterSelector {
    val code = 0xc8

    implicit val parameterCodec: ParameterCodec[BoardRevision.type, ShortString, Nothing] =
      ParameterCodec.parameterCodecFor[BoardRevision.type, ShortString]
  }

  case object PlatformModelName extends ParameterSelector {
    val code = 0xd1

    implicit val parameterCodec: ParameterCodec[PlatformModelName.type, BlockData, LongString] =
      ParameterCodec.blockParameterCodecFor[PlatformModelName.type, BlockData, LongString]
  }

  case object BladeSlotInfo extends ParameterSelector {
    val code = 0xdc

    implicit val parameterCodec: ParameterCodec[BladeSlotInfo.type, BlockData, LongString] =
      ParameterCodec.blockParameterCodecFor[BladeSlotInfo.type, BlockData, LongString]
  }

  case object iDracIpV4Url extends ParameterSelector {
    val code = 0xde

    implicit val parameterCodec: ParameterCodec[iDracIpV4Url.type, BlockData, LongString] =
      ParameterCodec.blockParameterCodecFor[iDracIpV4Url.type, BlockData, LongString]
  }

  case object CmcIpV4Url extends ParameterSelector {
    val code = 0xe0

    implicit val parameterCodec: ParameterCodec[CmcIpV4Url.type, BlockData, LongString] =
      ParameterCodec.blockParameterCodecFor[CmcIpV4Url.type, BlockData, LongString]
  }

  case object CmcIpV6Url extends ParameterSelector {
    val code = 0xf3

    implicit val parameterCodec: ParameterCodec[CmcIpV6Url.type, BlockData, LongString] =
      ParameterCodec.blockParameterCodecFor[CmcIpV6Url.type, BlockData, LongString]
  }
  ...
  case object IdracInfo extends ParameterSelector {
    ...
    implicit val parameterCodec: ParameterCodec[IdracInfo.type, BlockData, Data] =
      ParameterCodec.blockParameterCodecFor[IdracInfo.type, BlockData, Data]
  }
  ...
  case object MacAddressesFor10G extends ParameterSelector {
    ...
    implicit val parameterCodec: ParameterCodec[MacAddressesFor10G.type, MacAddresses, Nothing] =
      ParameterCodec.parameterCodecFor[MacAddressesFor10G.type, MacAddresses]

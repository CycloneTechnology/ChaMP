...
trait IpmiClient {
  ...
  def connectionFor(
    ...
    port: Int = IpmiTarget.defaultPort): Future[IpmiConnection]
  ...
  def withConnection[T](
    ...
    (task: IpmiConnection => Future[T]): Future[T] = {
...
trait IpmiConnection {
  /**
    ...
    */
  def negotiateSession(ipmiCredentials: IpmiCredentials,
  ...
  /**
    ...
    */
  def elevatePrivilegeLevel(privilegeLevel: PrivilegeLevel)
  ...
  def executeCommand[Command <: IpmiStandardCommand, Result <: IpmiCommandResult](
    command: Command, targetAddress: DeviceAddress)
  ...
  def executeCommandOrError[Command <: IpmiStandardCommand, Result <: IpmiCommandResult](
    command: Command, targetAddress: DeviceAddress)
    (implicit timeoutContext: TimeoutContext, codec: CommandResultCodec[Command, Result]): Future[IpmiErrorOr[Result]]

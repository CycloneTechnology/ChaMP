...
object SdrFilter {
  ...
  case class BySensorType(sensorType: SensorType) extends SdrFilter {
    val predicate: SdrKeyAndBody => Boolean = { record: SdrKeyAndBody => record.optSensorType.contains(sensorType) }
  }
  ...
  case object All extends SdrFilter {
    val predicate: SdrKeyAndBody => Boolean = { _: SdrKeyAndBody => true }
  }

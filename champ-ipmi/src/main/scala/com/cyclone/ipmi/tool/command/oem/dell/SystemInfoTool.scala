  ...
  object Command extends IpmiToolCommand with LazyLogging {
    implicit val executor: CommandExecutor[Command.type, Result] =
      new CommandExecutor[Command.type, Result] {
        ...
        def execute(command: Command.type)(implicit ctx: Ctx): Future[IpmiError \/ Result] = {
          ...
          val result = for {
            ...
            macAddresses <- iDracInfo match {
              case Some(info) if info.`type` == IdracType.`11G Modular` =>
                eitherT(macAddressFor11Or12G)
              case Some(info) if info.`type` == IdracType.`11G Monolithic` =>
                eitherT(macAddressFor11Or12G)
              case Some(info) if info.`type` == IdracType.`12G Modular` =>
                eitherT(macAddressFor11Or12G)
              case Some(info) if info.`type` == IdracType.`12G Monolithic` =>
                eitherT(macAddressFor11Or12G)
        ...
        private def macAddressFor11Or12G(
          implicit ctx: IpmiOperationContext
        ): Future[IpmiErrorOr[Option[MacAddresses]]] = {
          ...
          def getSize: Future[IpmiErrorOr[Int]] = {
            val result = for {
              cmdResult <- eitherT(
                connection
                  .executeCommandOrError(GetSystemInfoParametersEx.Command(MacAddressesFor11Or12G))
              )
            } yield {
            ...
            val result = for {
              cmdResult <- eitherT(
                connection.executeCommandOrError(
                  GetSystemInfoParametersEx.Command(
              ...
              logger.debug(
                s"Getting 11/12G mac address for offset $readOffset from ${cmdResult.responseData.toHexString()}"
              )

...
object SensorMasks {
  def decoder(
    ...
    sensorType: SensorType): Decoder[SensorMasks] = new Decoder[SensorMasks] {
        ...
        eventReadingType.category match {
          case EventReadingCategory.Threshold =>
            ReadingMask.Threshold(
              (data(0).bits4To6.toUnsignedInt + (data(2).bits4To6.toUnsignedInt << 3)).toByte.as[EventBits])

          case EventReadingCategory.Discrete =>
            ReadingMask.DiscreteStates(
              ByteString(data(4), data(5)).as[EventBits], eventReadingType)

          case EventReadingCategory.SensorSpecific =>
            ReadingMask.SensorSpecificDiscreteStates(
              ByteString(data(4), data(5)).as[EventBits], sensorType)

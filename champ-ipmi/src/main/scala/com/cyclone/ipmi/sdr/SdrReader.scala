...
trait SdrReader {
  /**
    ...
    */
  def readSdrHeader(recordId: SensorDataRecordId, reservationId: SdrReservationId = SdrReservationId.noReservation)
  ...
  def readAllSdrHeaders(reservationId: SdrReservationId = SdrReservationId.noReservation)
    (implicit ctx: IpmiOperationContext): Future[IpmiErrorOr[Seq[SdrHeader]]]

  /**
    * Reads an SDR with a specific record id
    */
  def readSdr(recordId: SensorDataRecordId)
    (implicit ctx: IpmiOperationContext): Future[IpmiErrorOr[SdrKeyAndBody]]
...
object SdrReaderComponent extends SdrReaderComponent with LazyLogging {
  ...
  lazy val sdrReader: SdrReader = new SdrReader {
    ...
    def readSdrHeader(recordId: SensorDataRecordId, reservationId: SdrReservationId)
      (implicit ctx: IpmiOperationContext): Future[IpmiError \/ SdrHeader] = {

      logger.debug(s"Reading SDR header for $recordId, $reservationId")

      val result = for {
        commandResult <- runGetSdr(recordId, reservationId, bytesToRead = SensorDataRecord.headerLength)
    ...
    def readAllSdrHeaders(reservationId: SdrReservationId)(implicit ctx: IpmiOperationContext): Future[IpmiError \/ Seq[SdrHeader]] = {
      def loop(
        acc: Vector[SdrHeader],
        recordId: SensorDataRecordId): FutureIpmiErrorOr[Seq[SdrHeader]] =
    ...
    def readSdr(recordId: SensorDataRecordId)(implicit ctx: IpmiOperationContext): Future[IpmiError \/ SdrKeyAndBody] = {
      val result = for {
        reservation <- eitherT(getReservation)
    ...
    private def readAllSdrBodies(reservationId: SdrReservationId)
      (implicit ctx: IpmiOperationContext): Future[IpmiErrorOr[Seq[SdrKeyAndBody]]] = {

      def loop(
        acc: Vector[SdrKeyAndBody],
        recordId: SensorDataRecordId): FutureIpmiErrorOr[Seq[SdrKeyAndBody]] =
    ...
    private def readSdrBody(recordId: SensorDataRecordId, reservationId: SdrReservationId)
      (implicit ctx: IpmiOperationContext): Future[IpmiErrorOr[(SdrKeyAndBody, SensorDataRecordId)]] = {
      import GenericStatusCodeErrors._

      def doReadSdrBody(remainingToChunkSize: Int => Int): Future[IpmiErrorOr[(SdrKeyAndBody, SensorDataRecordId)]] = {

        def loop(acc: ByteString, totalLength: Int, offset: Int, bytesToRead: Int): FutureIpmiErrorOr[ByteString] = {
          logger.debug(s"Getting SDR chunk from $offset to ${offset + bytesToRead} (of $totalLength).")
          if (bytesToRead <= 0) eitherT(acc.right[IpmiError].point[Future])
            ...
            for {
              ...
              result <- loop(newAcc, totalLength, offset = offset + data.length, bytesToRead = remaining min chunkSize)
            } yield result
        }
        ...
        val result = for {
          ...
          sensorDataRecordBytes <- loop(
            header.headerData, header.totalLength,
            SensorDataRecord.headerLength, remainingToChunkSize(remaining))
      ...
      val inChunks = { remaining: Int => remaining min chunkSize }
      val inOneGo = { remaining: Int => remaining }
      ...
      doReadSdrBody(inOneGo).flatMap {
        ...
        case -\/(e)
          if e == TooManyBytesRequested |
            e == UnspecifiedError => doReadSdrBody(inChunks)
        case -\/(e)               => e.left.point[Future]
      }
    }

    private def runGetSdr(
      recordId: SensorDataRecordId, reservationId: SdrReservationId,
      offset: Int = 0, bytesToRead: Int = 0xff)
      (implicit ctx: IpmiOperationContext) = {
      implicit val timeoutContext: TimeoutContext = ctx.timeoutContext
      import ctx._

      for {
        commandResult <- eitherT(connection.executeCommandOrError(GetSDR.Command(reservationId, recordId, offset, bytesToRead)))
        _ = logger.debug(s"Read SDR chunk from $offset for $bytesToRead bytes for $recordId: $commandResult")
      } yield commandResult
    }

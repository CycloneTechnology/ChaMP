...
case class ListPropertyValue[V <: PropertyValue](values: List[V]) extends PropertyValue {
  require(
    requirement = !values.exists(_.isInstanceOf[ListPropertyValue[_]]),
    message = "Can't wrap list properties with list properties"
  )

  require(
    values == Nil || values.map(_.getClass).toSet.size == 1,
    "Heterogeneous list properties are not permitted"
  )

  def containsSimpleValues: Boolean =
    values != Nil && values.head.getClass == classOf[StringPropertyValue]
}

object ListPropertyValue {

  @annotation.varargs
  def fromStrings(values: String*): ListPropertyValue[StringPropertyValue] =
    ListPropertyValue(values.map(v => StringPropertyValue(v)).toList)

...
private[wsman] object WSManOperations extends ReferenceResolver {
  def get(ref: ManagedReference, deadline: OperationDeadline)
    ...
    val result = for {
      response <- eitherT(context.connection.executeSoapRequest(
        GetXML(ref, ref.selectorClause, deadline)))
  ...
  /**
    ...
    */
  def determineAvailability(deadline: OperationDeadline)
  ...
  /**
    ...
    */
  def identify(deadline: OperationDeadline)
  ...
  /**
    * Creates the specified instance on the device.
    */
  def create(inst: ManagedInstance,
    resourceURI: String, cimNamespace: Option[String], deadline: OperationDeadline)
    (implicit context: WSManOperationContext): Future[WSManErrorOr[ManagedReference]] = {
    val result = for {
      response <- eitherT(context.connection.executeSoapRequest(
        CreateXML(
  ...
  def delete(ref: ManagedReference, deadline: OperationDeadline)
    (implicit context: WSManOperationContext): Future[WSManErrorOr[Unit]] = {
    val result = for {
      _ <- eitherT(context.connection

...
object Get {
  ...
  def fromClassName(className: String,
    ...
    selectorClause: SelectorClause = SelectorClause.empty,
    resolveReferences: Boolean = false,
    cimNamespace: Option[String] = None): Get =
    Get(
  ...
  implicit object Executor extends WSManQueryDefn.Executor[Get] {
    ...
    protected def executeInstancesQuery(
      query: Get, ref: ManagedReference)
      ...
      val result = for (
        managedInstance <- eitherT(WSManOperations.get(refWithSelectors, context.operationDeadline))

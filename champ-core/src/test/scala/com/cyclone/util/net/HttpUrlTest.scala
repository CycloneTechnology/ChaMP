  ...
  "HttpUrl" when {
    "getting qualified host in domain" must {
      "look up FQDN when domain and address specified" in {
        HttpUrl.fromParts(fromString(domain1Address), "/path", ssl = false)
          .withQualifiedHostNameIfInDomain(Some(domain)).futureValue shouldBe
          HttpUrl.fromString(s"http://$domain1FQDN/path")
      }

      "look up FQDN when domain and unqualified host specified" in {
        HttpUrl.fromParts(fromString(domain1Host), "/path", ssl = false)
          .withQualifiedHostNameIfInDomain(Some(domain)).futureValue shouldBe
          HttpUrl.fromString(s"http://$domain1FQDN/path")
      }

      "use existing FQDN when domain and FQDN specified" in {
        HttpUrl.fromParts(fromString(domain1FQDN), "/path", ssl = false)
          .withQualifiedHostNameIfInDomain(Some(domain)).futureValue shouldBe
          HttpUrl.fromString(s"http://$domain1FQDN/path")
      }

      "use specified host if lookup fails" in {
        HttpUrl.fromParts(fromString("someHost"), "/path", ssl = false)
          .withQualifiedHostNameIfInDomain(Some(domain)).futureValue shouldBe
          HttpUrl.fromString("http://someHost/path")
      }

      "use host matching domain when address resolved to multiple host names" in {
        HttpUrl.fromParts(fromString("10.0.0.6"), "/path", ssl = false)
          .withQualifiedHostNameIfInDomain(Some(domain)).futureValue shouldBe
      ...
      "use first host unqualified resolves to multiple host names" in {
        HttpUrl.fromParts(fromString("mail"), "/path", ssl = false)
          .withQualifiedHostNameIfInDomain(None).futureValue should
